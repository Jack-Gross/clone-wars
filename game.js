...
function frame(timestamp) {
  ...
  if (state === 'playing') {
    ...
    const hitsBoundary = birdBottom >= CONFIG.canvasHeight - CONFIG.groundHeight || birdTop <= 0;
    const hitsPipe = pipes.some((pipe) => {
      const overlapsX = birdRight > pipe.x && birdLeft < pipe.x + CONFIG.pipeWidth;
      const overlapsY = birdTop < pipe.gapTop || birdBottom > pipe.gapBottom;
      return overlapsX && overlapsY;
    });
    if (hitsBoundary || hitsPipe) crash();
